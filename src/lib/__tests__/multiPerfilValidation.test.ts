...
describe("Bateria de Testes Multi-Perfil", () => {
  it("Perfil 1: Sedentária, IMC Alto (90kg), 90min - Foco em Segurança e Volume", () => {
    ...
    console.log(
      `[Perfil 1] BMR Esperada: ${bmr} kcal. A IA não deve prescrever menos que isso.`
    );
  ...
  it("Perfil 2: Atleta, 45min, Ganho de Massa - Foco em Intensidade e Tempo", () => {
    ...
    console.log(
      `[Perfil 2] Exercícios Push (45min): ${pushDay?.exercises.length}`
    );
  ...
  it("Perfil 3: Moderado, 30min, Emagrecimento - Foco em Eficiência Extrema", () => {
    ...
    plan.weeklySchedule.forEach((day) => {
      console.log(
        `[Perfil 3] Exercícios ${day.type} (30min): ${day.exercises.length}`
      );
      // Em 30 minutos, permitimos até 6 exercícios para garantir cobertura mínima (Shock Volume + Grupos Grandes)
  ...
  it("Verificação de Variedade A/B (Sem repetição)", () => {
    ...
    console.log(
      `[Variedade] Exercícios repetidos entre Upper A e B: ${intersection.length}`
    );

...
async function runExhaustiveTest() {
  ...
  console.log(
    "| Perfil | Exercícios/Dia | Séries/Ex | Tempo Est. | Variedade A/B |"
  );
  console.log(
    "|--------|----------------|-----------|------------|---------------|"
  );
  ...
  for (const p of profilesToTest) {
    ...
    const avgSets = dayA
      ? (
          dayA.exercises.reduce((acc, ex) => acc + ex.sets, 0) /
          dayA.exercises.length
        ).toFixed(1)
      : 0;
    ...
    const varietyScore = dayA
      ? `${namesA.size - intersection.length}/${namesA.size}`
      : "N/A";
    ...
    console.log(
      `| ${p.label.padEnd(18)} | ${dayA?.exercises.length.toString().padEnd(14)} | ${avgSets.toString().padEnd(9)} | ${estimatedTime.toString().padEnd(10)} | ${varietyScore.padEnd(13)} |`
    );
  }
  console.log(
    "================================================================"
...
interface LiteExercise {
  sets: number;
  rest: string;
}

function calculateTime(exercises: LiteExercise[]): number {
  let totalSeconds = 0;

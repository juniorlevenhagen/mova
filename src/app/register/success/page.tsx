...
function SuccessPageContent() {
  ...
  useEffect(() => {
    ...
    if (sessionId) {
      // Verificar se é um upgrade (usuário já logado) ou registro novo
      const checkUserAndRedirect = async () => {
        try {
          // Verificar se já tem usuário logado (upgrade)
          const {
            data: { user },
          } = await supabase.auth.getUser();

          if (user) {
            // ✅ Usuário logado fazendo upgrade - redirecionar para dashboard com parâmetro
            router.push("/dashboard?upgrade=success");
            return;
          }

          // Novo registro - tentar fazer login automático
